...
export function createTelegramBot(options: {
  ...
  bot.command("help", async (ctx) => {
    const keywordsUrl = keywordSheetUrl(config.spreadsheetId);
    await ctx.reply(
      [
        "🤖 <b>Playroom SEO bot</b>",
        "",
        "Я создаю и публикую SEO-статьи из очереди Google Sheets. Одновременно в работе может быть только одна статья.",
        "",
        "<b>Команды</b>",
        "",
        ...
        "",
        "По понедельникам и пятницам в 10:00 по Белграду я проверяю очередь и создаю одну статью. Плановый запуск берёт ready-строку с наибольшим priority, а при равном priority — верхнюю.",
        `Если готовых ключевиков нет, я предупрежу группу и дам <a href="${escapeHtml(keywordsUrl)}">ссылку на лист keywords</a>.`,
        "",
